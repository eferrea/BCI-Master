classdef decoder_class < handle;
    ...
    properties (SetAccess = ?calibrator_class)
        preferred_direction;
        baseline_rate;
        modulation_depth;
    end
    ...
    methods (Access=public)
        ...
        function obj = decoder_class(number_dimensions)
        ...
        function obj = loop(obj,task_state,t,spike_data)
            ...
            if ~isempty(task_state.new_stage) && (strcmp(task_state.stage_type,'LEAVE_FIX_MEM') || strcmp(task_state.stage_type,'PRE_ACQUIRE_MEM_TGT') || strcmp(task_state.stage_type,'ACQUIRE_MEM_TGT')) 
                ...
                obj.update_velocity(selected_spikes);
                 obj.update_position();
...
%            obj.time_counter = t(end) + obj.iteration_step;
        ...
        function obj = update_velocity(obj,firing_rate)
