function FakeMonkey(ismonkey,neurons,dimensions,delay,use_eye_fixation)
...
BCI_update_time = 0.050 %in secs. As described in papers KF usually uses 50 ms.
counter = 1;
...
direction_vector = [0 0 0]; %to store directions of the target
velocity_vector = zeros(ceil(max_trial_duration/BCI_update_time),3); % used to simulate Poisson like
position_vector = zeros(ceil(max_trial_duration/BCI_update_time),3);
accelleration_vector = zeros(ceil(max_trial_duration/BCI_update_time),3);
...
% % %initialize VRPN matlab client
vrpn_client('open_connection','Tracker20@172.17.6.10:6666') %same computer
% % %vrpn_client('open_connection','Tracker10@127.0.0.1:6666') %same computer
...
% p1 = uipanel('Title','Shared Control','FontSize',8,'Position',[0.89 0.25 .1 .25])
%
% h1 = uicontrol(p1,'Style', 'PushButton', 'String', '100 %','Units','normalized',...
%     'Callback', @Full_SC,'Position',[0 .01 1 .12]);
% %Configure Switch window
% g1 = uicontrol(p1,'Style', 'PushButton', 'String', '80 %', 'Units','normalized',...
%     'Callback',@High_SC,'Position',[0 .13 1 .12]);
%
% v1 = uicontrol(p1,'Style', 'PushButton', 'String', '60 %', 'Units','normalized',...
%     'Callback',@Medium_SC,'Position',[0 .25 1 .12]);
%
% f1 = uicontrol(p1,'Style', 'PushButton', 'String', '40 %', 'Units','normalized',...
%     'Callback',@Partial_SC,'Position',[0 .37 1 .12]);
% b1 = uicontrol(p1,'Style', 'PushButton', 'String', '30 %', 'Units','normalized',...
%     'Callback',@Mild_SC,'Position',[0 .49 1 .12]);
%
% u1 = uicontrol(p1,'Style', 'PushButton', 'String', '20 %', 'Units','normalized',...
%     'Callback',@Low_SC,'Position',[0 .61 1 .12]);
%
% t1 = uicontrol(p1,'Style', 'PushButton', 'String', '10 %', 'Units','normalized',...
%     'Callback',@Moderate_SC,'Position',[0 .73 1 .12]);
% s1 = uicontrol(p1,'Style', 'PushButton', 'String', '0 %', 'Units','normalized',...
%     'Callback',@None_SC,'Position',[0 .85 1 .12]);

% l  = uicontrol(p,'Style', 'PushButton', 'String', 'Calc Reg.','Units','normalized',...
%     'Callback', @CalculateRegression,'Position',[0 .69 1 .12]);
...
%log_file = ['n_neurons:' num2str(neurons) '  dimensions:' num2str(dimensions) '  delay:' num2str(delay)];
%  vrpn_server('send_message',['n_neurons:' num2str(neurons)])
%   vrpn_server('send_message',['delay:' num2str(delay)])
%   vrpn_server('send_message',['dimension:' num2str(dimensions)])
% cis = 'suca'
%   vrpn_server('send_message',cis)
...
while (task_running)
    ...
    a=vrpn_client('get_positions','Tracker20@172.17.6.10:6666'); %same
    %computer local
    %a=vrpn_client('get_positions','Tracker10@172.17.6.10:6666'); %my computer
    
    %read task controller state
    [b,t]=vrpn_client('get_messages','Tracker20@172.17.6.10:6666'); %same
    %computer local
    ...
    %     if (str2double(tp.iseye) == eye_fixation )
    %         disp(tp.iseye)
    %
    %     end
    ...
    if ~isempty(a)
        ...
        position_vector(counter,:) =  a(end,:)*1000;%the counting is done on a trial basis.
...
% function Full_SC(hObj,event)
%       perc = 1
% end
% function High_SC(hObj,event)
%       perc = 0.8
% end
% function Medium_SC(hObj,event)
%       perc = 0.6
% end
% function Partial_SC(hObj,event)
%       perc = 0.4
% end
% function Mild_SC(hObj,event)
%       perc = 0.3
% end
% function Low_SC(hObj,event)
%       perc = 0.2
% end
% function Moderate_SC(hObj,event)
%       perc = 0.1
% end
%
% function None_SC(hObj,event)
%       perc = 0
% end
