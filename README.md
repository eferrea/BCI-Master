...
*Fig 1.	The GUI layout reflects our experimental settings.  We used six electrode arrays each of them containing 32 channels (total 128 of recording sites). The online recorded spikes were streamed with 128 channel Cerebus recording system (Blackrock Microsystems, Salt Lake City, USA, https://www.blackrockmicro.com/). In the GUI, the identity of the channel is arranged as column while each row represent a different unit for that channel. The tested  recording system streams up to six units per channel that can be identified online. The table in the middle show the tuning properties of the selected unit by maintaining the same numenclature as explained previously (colum = channel identity, row = unit identity)* .
